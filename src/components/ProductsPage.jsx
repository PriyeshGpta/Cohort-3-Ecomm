      ...
      {products.map((product) => (
        <div className="bg-gray-400 rounded-md p-3" key={product.id}>
          ...
          {getProductQuantity(product.id) ? (
            <div className="flex justify-between mt-2 px-4 py-2 rounded bg-yellow-500 active:scale-98 transition-all duration-300">
              ...
              <button className="cursor-pointer">
                <Plus size={20} onClick={() => handleAddToCart(product.id)} />
              </button>
            </div>
          ) : (
            <button
